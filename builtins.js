function fillGlobalEnv(env, exec) {
  ...
  class Base {
    ...
    get value() {
      if (this.valueExists("__value")) {
        if (!this.valueExists("__is")) error("'__value' block requires the existence of an '__is' block in the same object.", false, true)
        ...
        if (!(res.is(this.getName()))) error("Return value of '__value' block must be of the type specified by '__is'.", false, true)
    ...
    setThisValue(name, value) {
      if (this.primitive) error(`Cannot set a property of a primitive (${this.name}).`, false, true)
    ...
    func(name, params = [], th) { return this.getValue(name, true).call(th || this, params) }
  }
  ...
  */
  ...
  builtIns.Function = class extends Base {
    constructor(params, body, environment, thisArg) {
      super()
      this.params = params
      this.body = body
    ...
    call(thisArg, params) {
      ...
      if (this.body instanceof Function) returnVal = this.body(env, params)
        ...
        returnVal = exec.execBlock(
          ...
          true,      // code is in function      v parameters v
          Object.fromEntries(params.map((value, i) => [this.params[i].name, value])),
  ...
  */
  ...
  function builtInFunc(field, name, func, minParams = 0) {
    fields[field][name] = new builtIns.Function(minParams ? Array(minParams).fill(null) : [], func)
  }

  builtInFunc("Object", "stringName", th => new builtIns.String(th.name))
  builtInFunc("Object", "stringPath", (th, params) => {
    var delimiter = "."
    if (params.length)
      if (params[0].is("String")) delimiter = params[0].value
      else error("Delimiter for [Object].stringPath must be a string or blank.", false, true)
  ...
  builtInFunc("Object", "isString", () => new builtIns.Boolean(false))
  builtInFunc("Object", "isNumber", () => new builtIns.Boolean(false))
  builtInFunc("Object", "isBoolean", () => new builtIns.Boolean(false))
  builtInFunc("Object", "isNull", () => new builtIns.Boolean(false))
  builtInFunc("Object", "isHash", () => new builtIns.Boolean(false))
  builtInFunc("Object", "isFunction", () => new builtIns.Boolean(false))
  builtInFunc("Object", "isBlock", () => new builtIns.Boolean(false))
  builtInFunc("Object", "isClass", () => new builtIns.Boolean(false))
  builtInFunc("Object", "toString", th => new builtIns.String(`[${th.name}]`))
  builtInFunc("Object", "toBoolean", () => new builtIns.Boolean(true))
  builtInFunc("Object", "type", th => new builtIns.String(th.getName().toLowerCase()))
  builtInFunc("Object", "trueType", th => new builtIns.String(th.name.toLowerCase()))
  builtInFunc("Object", "properties", th => new builtIns.Array(Object.keys(th.data).map(x => new builtIns.String(x))))
  builtInFunc("Object", "propertyValues", th => new builtIns.Array(Object.values(th.data)))
